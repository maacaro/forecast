  ...
  componentDidMount = async () => {
    try {
      const lastSearches = JSON.parse(
        await AsyncStorage.getItem("lastSearches")
      );
  ...
  setLastSearchesInAsyncStorage = async lastSearches => {
    try {
      await AsyncStorage.setItem(
        "lastSearches",
        JSON.stringify(this.state.lastSearches)
      );
    } catch (error) {
      console.log(error);
    }
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <LastSearches
          data={this.state.lastSearches}
          deleteLastSearchesItem={this.deleteLastSearchesItem}
