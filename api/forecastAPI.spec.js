...
describe("getForecast", () => {
  ...
  describe("the request", () => {
    it("should make the request to the proper URL endpoint", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        "http://api.openweathermap.org/data/2.5/weather?q=London&appid=6a3c56089632a3cd9af5bd986a812c22"
      );
    });
    ...
    it("should make the request with the city passed by arguments to `getForecast`", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        "http://api.openweathermap.org/data/2.5/weather?q=miami&appid=6a3c56089632a3cd9af5bd986a812c22"
      );
    });
  });
  ...
  describe("the response", () => {
    it("should containg object with main issues and location coord.", async () => {
      ...
      fetch.mockImplementationOnce(() =>
        Promise.resolve({
          json: jest.fn(() => Promise.resolve(endpointResponse))
        })
      );
      const response = {
        city: "London",
